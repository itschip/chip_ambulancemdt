...
async function createTreatment(treatment: ITreatment): Promise<any> {
  const query = "INSERT INTO or_ambu_treatments (name, date, note, doctor) VALUES (?, ?, ?, ?)"
...
async function fetchScopedTreatments(playerName: string): Promise<ITreatment[]> {
...
async function updateTreatments(treatment: ITreatment) {
  const query = "UPDATE or_ambu_treatments SET name = ?, date = ?, note = ?, doctor = ? WHERE id = ?";
  await pool.query(query, [
    treatment.name, 
    treatment.date,
    treatment.note, 
