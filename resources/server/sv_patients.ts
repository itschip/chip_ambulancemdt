...
async function getFilteredPatients(search: string): Promise<any> {
  const query = "SELECT firstname, lastname, dateofbirth, bloodtype, avatar, phone_number, doctor FROM users WHERE lower(firstname) LIKE lower(?) OR lower(lastname) LIKE lower(?)";
...
async function updatePatientCredentials(credentials: IPatient) {
  const query = "UPDATE users SET bloodtype = ?, avatar = ?, doctor = ? WHERE firstname = ? AND lastname = ?";
